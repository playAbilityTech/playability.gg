...
const setKeyState = (key, state, device = 0) => {
    if (!inputData[device]) {
      inputData[device] = [];
    }
    inputData[device][key] = state;
    console.log('setKeyState', key, state, device);
  };
  ...
  const emulateKeyRelease = (key, device = 0) => {
    setKeyState(key, 4, device); // 4 (released)
  };
  
// Expose the functions to the window object so they can be accessed from the React component
window.emulateKeyPress = emulateKeyPress;
window.emulateKeyRelease = emulateKeyRelease;
